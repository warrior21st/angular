var Menus= React.createClass({
    ...
    render:function(){
        ...
        for(var i=0;i<list.length;i++){
            var module=list[i];
            items.push(<div className="leftMnTab_title" data-toggle="collapse" data-parent="#leftmenus" data-target={"#child"+module.ID} _id={module.ID} >{module.title}</div>);
            var itemschild=[];
            for(var j=0;j<module.childs.length;j++){
                var func=module.childs[j];
                itemschild.push(<li className="leftMnTab_lt_liebiao" _id={"function"+func.ID} _title={func.title} _key={func.key} ui-sref={func.key} onClick={this.handleClick.bind(this,func.ID,func.title)}><span>{func.title}</span></li>);
            }
            items.push(<ul id={"child"+module.ID} className="collapse leftMnTab_lt">{itemschild}</ul>);
        }

        return(<div id="leftmenudiv" className="col-xs-12 panel leftMnTab">{items}</div>);
    }
});
